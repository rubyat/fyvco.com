    ...
    <div class="media d-flex">
        ...
        <div class="media-body">
            ...
            @if((!Auth::check() or Auth::id() != $row->author_id ) and setting_item('inbox_enable'))
                <a class="btn bc_start_chat btn bc_start_chat button -dark-1 py-5  rounded-4 bg-blue-1 text-white cursor-pointer btn-primary" href="{{route('user.chat',['user_id'=>$row->author_id])}}" ><i class="fa fa-wechat me-1"></i> {{__('Message host')}}</a>
            @endif
        </div>
    </div>
</div>
