...
@else
   <div class="pickup_return">
        ...
        <div class="pick_ul_loc_title">
            <h4 class="text-15 fw-500 ls-2 lh-16">{{ $field['title'] }}</h4>
        </div>
        <div class="pickup_form_area d-flex">
            ...
            <div class="searchMenu-loc js-form-dd js-liverSearch item return_location">
                <span class="clear-loc absolute bottom-0 text-12"><i class="icon-close"></i></span>
                <div data-x-dd-click="searchMenu-loc">
                    <div class="text-15 text-light-1 ls-2 lh-16  @if( $type == "autocomplete") smart-search  @endif ">
                        <input type="hidden" name="{{$inputName}}-return" class="js-search-get-id child_id" value="{{ $location_id ?? '' }}">
                        <input type="text" autocomplete="off" @if( $type == "normal") readonly  @endif class="smart-search-location parent_text js-search js-dd-focus" placeholder="{{__("+ Different return location")}}" value="{{ $location_name }}" data-onLoad="{{__("Loading...")}}" data-default="{{ json_encode($list_json) }}">
                    </div>
                </div>
                <div class="searchMenu-loc__field shadow-2 js-popup-window @if( $type == "autocomplete") d-none @endif" data-x-dd="searchMenu-loc" data-x-dd-toggle="-is-active">
                    <div class="bg-white px-30 py-30 sm:px-0 sm:py-15 rounded-4">
