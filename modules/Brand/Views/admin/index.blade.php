...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Brand")}}</h1>
        </div>
        ...
        <div class="row">
                    ...
                    <div class="panel-body panel-index">
                        <form action="{{route('brand.admin.store',['id'=>'-1','lang'=>request()->query('lang')])}}" method="post">
                            ...
                            <div class="form-group form-index-hide d-none">
                                ...
                                <p><i>{{__('Click onto map to place Brand address')}}</i></p>
                            </div>
                            @php do_action(\Modules\Brand\Hook::AFTER_MAP,$row) @endphp
                            <div class="">
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{route("brand.admin.bulkEdit")}}"
                    ...
                    <div class="col-left">
                        <form method="get" action="{{route('brand.admin.index')}}" class="filter-form filter-form-right d-flex justify-content-end" role="search">
                            ...
                            <table class="table table-hover">
                                ...
                                @if( count($rows) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="title">
                                            <a href="{{route('brand.admin.edit',['id'=>$row->id])}}">{{$prefix.' '.$row->name}}</a>
                                        </td>
                                        <td>{{$row->slug}}</td>
                                        <td><span class="badge badge-{{ $row->status }}">{{ $row->status }}</span></td>
                                        <td>{{display_date($row->updated_at)}}</td>
                                        <td>
                                            <a href="{{route('brand.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}
                                            </a>
                                        </td>
                                    </tr>
                                    <?php
